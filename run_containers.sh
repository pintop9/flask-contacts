...
docker run -d --name contacts-db -e POSTGRES_DB=contacts_db -e POSTGRES_USER=contacts_user -e POSTGRES_PASSWORD=contacts_pass pintop9/contacts-db
...
docker run -d --name contacts-web --link contacts-db:db -p 5000:5000 pintop9/contacts-web
